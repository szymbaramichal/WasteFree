...
public sealed record UtilizationFeePendingNotificationRequest(
    Guid UserId,
...
public sealed class UtilizationFeePendingNotificationFacade(ApplicationDataContext context)
{
    public async Task<IReadOnlyList<UtilizationFeeNotificationContent>> CreateAsync(
        IEnumerable<UtilizationFeePendingNotificationRequest> requests,
        CancellationToken cancellationToken)
    {
        ...
        var templates = await context.NotificationTemplates
            .AsNoTracking()
            .Where(t => t.Type == NotificationType.UtilizationFeePending
    ...
    private static IReadOnlyDictionary<string, string> BuildPlaceholders(
        UtilizationFeePendingNotificationRequest request)
    {
        return new Dictionary<string, string>
        {
            ...
            ["OutstandingAmount"] = request.OutstandingAmount.ToString("F2", CultureInfo.InvariantCulture),
            ["UserShare"] = request.ShareAmount.ToString("F2", CultureInfo.InvariantCulture)
        };
    }
}
