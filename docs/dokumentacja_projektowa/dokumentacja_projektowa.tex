...
\item \textbf{Obserwowalność}: logowanie strukturalne, możliwość rozszerzenia o metryki (Prometheus) i centralne logi; korelacja żądań przez identyfikatory w middleware.
...
\item \textbf{Frontend: Angular 16/17} — sprawdzony framework z wbudowanym routingiem, DI i wsparciem dla i18n. Silne typowanie (TypeScript) ogranicza klasy błędów, a ekosystem narzędzi (CLI, testy) przyspiesza development zespołowy.
\item \textbf{Konteneryzacja: Docker} — hermetyzuje runtime (.NET, Node), upraszcza parytet środowisk (dev/stage/prod) i jest podstawą pipeline'ów CI/CD. Obrazy są przenośne między lokalnym Compose a produkcyjnym daemonem na Mikr.us.
...
\begin{itemize}
\item \textbf{wastefree-api} — obraz budowany z katalogu \texttt{API}, ekspozycja \texttt{5001:5001}, zmienne: \texttt{ASPNETCORE\_ENVIRONMENT=Docker} i \texttt{ASPNETCORE\_URLS=http://+:5001}. API korzysta z SQLite skonfigurowanego w profilu Docker.
\item \textbf{wastefree-ui} — obraz budowany z katalogu \texttt{UI}, ekspozycja \texttt{4200:80}, zależność od \textbf{wastefree-api}. UI komunikuje się z API pod adresem hosta (np. \texttt{http://localhost:5001}).
\end{itemize}
